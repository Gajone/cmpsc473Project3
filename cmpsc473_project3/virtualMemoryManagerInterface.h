...
public:
	...
	void swap(unsigned int frameNumber, unsigned int pageNumber) {
		///@todo ta instrumentation to go here ///students should instrument memorymanager performance in their ownclass for their internal verification or may
			///modify this code for their testing purpose; however the TAs instrumentation will used for determining grade basis.
			numSwaps++;
	} /** Report the to-date number of page swaps, used by test benches to
	  quantify performance.
	  * @returns the number of page swaps */
	unsigned long long& numberPageSwaps() { return numSwaps; }
protected:
	ReplacementPolicy policy; unsigned int N; /// N = Page Address bits size, 2^ N = The physical frame / virtual page size to manage(bytes)
